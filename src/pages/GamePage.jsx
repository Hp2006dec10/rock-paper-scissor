...
const ScoreBoard = ({hScore, cScore}) => {
    return (
        <div className="h-100 w-1200 justify-self-center mt-30 -outline-offset-7 outline-3 outline-dotted bg-black text-white">        
...
const GamePage = () => {
    ...
    useEffect(()=>{
        ...
        setTimeout(()=>{
            console.log("Score updated", humanInput, computerInput);
            setMessage("Start");
            setHuman(null);
            setComputer(null);
    ...
    return (
    <>
   <div className={`bg-[rgb(189,62,153)] h-screen w-screen ${gameOver ? "pointer-events-none opacity-50 cursor-not-allowed" : ""}`}>
...
const GameOver = ({winner, handleExit, handleRestart}) => {
    return (
        <div className="h-200 w-400 bg-[rgb(189,62,153)] absolute top-[calc(50%-100px)] left-[calc(50%-200px)]">
            ...
            <div className="flex justify-around w-400">
                <div className="text-center h-fit w-75 p-10 text-xl border-box bg-white cursor-pointer" onClick={handleRestart}>Restart</div>
                <div className="text-center h-fit w-75 p-10 text-xl border-box bg-white cursor-pointer" onClick={handleExit}>Exit</div>
            </div>
        </div>
    )
}
